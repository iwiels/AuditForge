...
var auditProfiles = []model.AuditProfile{
	...
	{
		ID:          model.AuditProfileRedTeam,
		Name:        "Adversarial Emulation",
		Description: "Explotación activa, post-explotación acotada, fuzzing pesado y validación de vulnerabilidades mediante weaponization.",
		FocusAreas:  []string{"exploitation", "weaponization", "deep fuzzing", "active validation"},
		Components: []model.ComponentID{
			model.ComponentMCP,
			model.ComponentPrompts,
			model.ComponentCommands,
		...
		Skills:       []model.SkillID{model.SkillAuthorizationGuard, model.SkillNetworkRecon, model.SkillParamDiscoveryFuzz, model.SkillThreatModeling, model.SkillSQLIHypothesisValidate, model.SkillAdvancedAuthBypass, model.SkillFileUploadAttacks, model.SkillDeserializationAttacks},
		Commands:     []string{"security-scout", "network-recon", "deep-web", "sqli-validate", "memory-search", "orquestador-auditor"},
		SubAgents:    append([]string(nil), allSpecialistSubAgents...),
		NativeAgents: append([]string(nil), allSpecialistNativeAgents...),
		Risk: model.RiskPolicy{
			Mode:           "active-exploitation",
			Summary:        "Permite explotación activa y fuzzing intensivo. REQUIERE autorización estricta.",
			AllowedActions: []string{"explotación activa", "fuzzing pesado", "generación de payloads", "interacción de proxy en tiempo real", "validación agresiva de vulnerabilidades"},
			BlockedActions: []string{"denegación de servicio (DoS)", "modificación persistente destructiva sin rollback"},
			Permissions:    model.ToolPermissions{Read: true, Write: true, Edit: true, Bash: true},
		},
	},
}
